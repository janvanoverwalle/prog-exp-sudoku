...
public class SudokuPanel extends JPanel {
	...
	/**
	 * Sets the sudoku to a given preset.
	...
	public void setSudoku(Sudoku s) {
		...
		for (int r = 0; r < Sudoku.GRID_SIZE; r++) {
			for (int c = 0; c < Sudoku.GRID_SIZE; c++) {
				int value = sudoku.getSquare(r, c);
				
				if (legalNumber(value)) {
					textFields[r][c].setText(Integer.toString(value));
				}
			}
		}
	}
	...
	public void solve() {
		...
		if (notANumber) {
			...
			if (solved) {
				...
				for (int r = 0; r < Sudoku.GRID_SIZE; r++) {
					for (int c = 0; c < Sudoku.GRID_SIZE; c++) {
						textFields[r][c].setText(Integer.toString(sudokuNumbers[r][c]));
					}
				}
			} else {
				JOptionPane.showMessageDialog(this,
